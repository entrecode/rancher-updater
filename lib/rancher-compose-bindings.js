...
const composeBindings = {
  ...
  .then(() => {
    const cmd = `rancher-compose -p ${argv.s} --url https://rancher.entrecode.de up -d -u --batch-size 1 --interval 5000`;
    return exec(cmd, {
      cwd: p,
      env: {
  ...
  .then(execRes => {
    ...
    return api.getStack(argv.e, argv.s);
  }),
  removeService: (serviceName) => Promise.resolve()
  .then(() => {
    const cmd = `rancher-compose -p ${argv.s} rm ${serviceName}`;
    return exec(cmd, {
      cwd: p,
      env: {
        RANCHER_ACCESS_KEY: argv.accessKey,
