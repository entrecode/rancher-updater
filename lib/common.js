...
const common = {
  ...
  .then((s) => {
    services = s;
    oldService = services.data.filter(service => service.name.startsWith(`${argv.stackName}-`));
    if (!oldService || oldService.length < 1) {
      return Promise.reject(new Error(`Old service name ${argv.v} not found. Abort.`));
    }
    if (oldService.length > 1) {
      return Promise.reject(new Error(`Found two old service names for ${argv.stackName}. Abort.`));
    }
