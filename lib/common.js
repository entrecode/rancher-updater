...
const common = {
  fail: (err) => {
    ...
    console.error(`Could not update stack ${argv.service} to version ${argv.version}.`);
    process.exit(1);
  },
  
  success: () => {
    compose.cleanup();
    console.info(`Sucessfully updated stack ${argv.service} to version ${argv.version}.`);
    process.exit(0);
  },
  
  getStack: () => api.getStack(argv.environment, argv.service)
  .then((s) => {
    stack = s;
  ...
  .then((stacks) => {
    const found = stacks.find(s => s.name === argv.service);
    if (found) {
      return Promise.reject(new Error(`Stack name ${argv.service} already taken.`));
    }
  ...
  checkHealth: () => api.getStack(argv.environment, argv.service)
  .then((s) => {
    stack = s;
    let timeout = 60000;
    return promiseWhile(
