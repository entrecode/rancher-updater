...
.describe('url', 'The url of the rancher server to connect to. With http(s):// please. Will be overwritten by RANCHER_URL environment variable')
.nargs('url', 1)
.default('url', 'https://rancher.entrecode.de')

.describe('balancer', 'The name of the load balancer service. Default: lb')
.nargs('balancer', 1)
.default('balancer', 'lb')

.describe('balancerVersion', 'Version of the load balancer image. Environment variable DEPLOY_BALANCER_VERSION can be used.')
.nargs('balancerVersion', 1)
.default('balancerVersion', 'v0.6.2')
...
.describe('mode', 'The updater mode. (\'balanced\', \'init\', \'initBalanced\', \'service\', \'inplace\'')
.alias('mode', 'm')
.nargs('mode', 1)
...
.describe('timetohealth', 'Timeout in ms to wait before consider service healthy.')
.alias('timetohealth', 'th')
.nargs('timetohealth', 1)
.number('timetohealth')
.default('timetohealth', 30000)
...
if (process.env.RANCHER_URL) {
  argv.url = process.env.RANCHER_URL;
}
